...
/*
    public function kopijuoti(){
        ...
        $this->form_validation->set_rules('nuo', 'NUO', 'required', array('required' => 'Nurodykite NUO'));
        $this->form_validation->set_rules('iki', 'IKI', 'required', array('required' => 'Nurodykite IKI'));
        ...
        if ($this->form_validation->run()) {
            ...
            $ukis = $this->ukininkai_model->ukininkas($sesija['nr']);
            if (count($ukis) == 0) {
            ...
            for ($i = 0; $i < 20; $i++) {
                ...
                $id = "100053" . $rodo;
                //$id = 1000544052;
                $url = "https://www.vic.lt:8102/pls/gris/private.details?v_id=" . $id;
                //duomenys kuriuos sius CURL
                $auth = $ukis[0]['VIC_vartotojo_vardas'] . ":" . $ukis[0]['VIC_slaptazodis'];
                //bandom nuskaityri duomenis
                $page = $this->galvijai_model->get_kopija($url, $auth);
                $data = $this->galvijai_model->kopija($page['content']);

                if (!$klaida) {
                    $aa = explode("\n", $data[3][0]);
                    //isgaunam Varda Pavarde:
                    $arr = explode(":", $aa[3]);
                    $vardas_pavarde = $arr[1];
                    if ($vardas_pavarde != "") {
                        //pirmas adresas, valdos nr
                        $keiciam = str_replace("Valda:", "#", $aa[4]);
                        $keiciam = str_replace("Adresas:", "#", $keiciam);
                        $keiciam = str_replace("gatvė:", "#", $keiciam);
                        $keiciam = str_replace("namas:", "#", $keiciam);
                        $keiciam = str_replace("butas:", "#", $keiciam);
                        $arr = explode("#", $keiciam);
                        //var_dump($arr);
                        $valdos_numeris = trim($arr[1]);
                        //pirmas adresas
                        $salinti = explode(" ", $arr[2]);
                        $kodas = $salinti[0];
                        if (is_numeric($kodas)) {
                            $adresas = trim(str_replace($salinti[0], " ", $arr[2]));
                        } else {
                            $adresas = trim($arr[2]);
                        }
                        $adr = explode(" ", $adresas);
                        ...
                        //antras adresas
                        $salinti_2 = explode(" ", $arr[5]);
                        $kodas_2 = $salinti_2[0];
                        if (is_numeric($kodas_2)) {
                            $adresas_2 = trim(str_replace($salinti_2[0], " ", $arr[5]));
                        } else {
                            $adresas_2 = trim($arr[5]);
                        }
                        $adr_2 = explode(" ", $adresas_2);
                        //$data_db['adresas_2']['miestas'] = $adr_2[0];

                        //tresias adresas
                        $salinti_3 = explode(" ", $arr[8]);
                        $kodas_3 = $salinti_3[0];
                        if (is_numeric($kodas_3)) {
                            $adresas_3 = trim(str_replace($salinti_3[0], " ", $arr[8]));
                        } else {
                            $adresas_3 = trim($arr[8]);
                        }
                        $adr_3 = explode(" ", $adresas_3);

                        //4 adresas
                        $salinti_4 = explode(" ", $arr[11]);
                        $kodas_4 = $salinti_4[0];
                        if (is_numeric($kodas_4)) {
                            $adresas_4 = trim(str_replace($salinti_4[0], " ", $arr[11]));
                        } else {
                            $adresas_4 = trim($arr[11]);
                        }
                        $adr_4 = explode(" ", $adresas_4);


                        $adresas = $adresas . " " . trim($arr[3]) . " " . trim($arr[4]) . " " . (is_numeric($arr[5]) ? $arr[5] : "");
                        $adresas_2 = $adresas_2 . " " . trim($arr[6]) . " " . trim($arr[7]) . " " . (is_numeric($arr[8]) ? $arr[8] : "");
                        $adresas_3 = $adresas_3 . " " . trim($arr[9]) . " " . trim($arr[10]) . " " . (is_numeric($arr[11]) ? $arr[11] : "");
                        $adresas_4 = $adresas_4 . " " . trim($arr[12]) . " " . trim($arr[13]) . " " . (is_numeric($arr[14]) ? $arr[14] : "");

                        $arr = explode(",", $aa[5]);
                        $ar = explode(":", $arr[0]);
                        $telefonas = $ar[1];

                        $kiek = $this->galvijai_model->tikinti_sarasa($valdos_numeris);
                        if ($kiek > 0) {
                            $this->main_model->info['error'][] = 'Ukininkas jau irasytas i DB!';
                        } else {
                            if ($adr[0]) {
                                $miestas = $adr[0];
                            } else if ($adr_2[0]) {
                            ...
                            $dat = array("valdos_numeris" => $valdos_numeris, "vardas_pavarde" => $vardas_pavarde, "telefonas" => $telefonas,
                                "adresas" => $adresas, "adresas_2" => $adresas_2, "adresas_3" => $adresas_3, "adresas_4" => $adresas_4, "miestas" => $miestas);
                            $this->galvijai_model->irasyti_sarasa($dat);
                            $this->main_model->info['error'][] = $rodo.': Įtraukti į duomenų bazę!';
                        }
                    } else {
                        $this->main_model->info['error'][] = 'Nera duomenu apie ukininka!';
                    }
                }
            }
        ...
        $this->load->view("galvijai/kopijuoti");
    }*/
