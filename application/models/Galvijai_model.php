...
/*
    public function get_kopija($url, $auth){
        $options = array(
    ...
    public function irasyti_sarasa($sql){
        $this->db->insert('ukininku_sarasas', $sql);
    }

    public function tikinti_sarasa($nr)
    {
        $this->db->select('id');
        $this->db->from('ukininku_sarasas');
        $this->db->where(array('valdos_numeris' => $nr));
        $result = $this->db->get();
